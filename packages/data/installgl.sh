...
until [ -n "$usetcpd" ]; do
    ...
    # case $usetcpd in
        ...
        # [Yy])
            ...
            usetcpd="y"
            ## SiCiaTeCh comments : Auto say 'y'... next
        # ;;
        # *)
            # unset usetcpd
            # continue
...
## SiCiaTeCh comments :Auto no jailed
until [ -n "$jail" ]; do
...
## SiCiaTeCh comments :Auto no jailed /end
...
## SiCiaTeCh comments : Disable; already ask.
...
## SiCiaTeCh comments : Already port select
...
## SiCiaTeCh comments : systemd is good
...
if [ ${#COUNT_SUPERSERVERS[@]} -eq 0 ]; then
    ...
    until [ -n "$whichnetd" ]; do
        ...
        ## SiCiaTeCh comments : auto select systemd 
        # read whichnetd
        whichnetd=s
...
## SiCiaTeCh comments : auto select 'y'
...
## SiCiaTeCh comments : auto set servicename for tlsname
  until [ -n "$tlsname" ]; do
    #read tlsname
    if [ -z "$tlsname" ]; then
      tlsname=$servicename
  ...
  echo ""
	...
	## SiCiaTeCh comments : certificat start
  ...
  # $opensslbin ecparam -out $base.key -name secp384r1 -genkey 2>&1
## SiCiaTeCh comments : certificat end
...
## SiCiaTeCh comments : change for multi conf
if [ "$netd" = "i" ]; then
    ...
    { grep -v ^${servicename} /etc/inetd.conf
        ...
        # echo "$servicename stream tcp6 nowait root $tcpd $jaildir$glroot/bin/glftpd $dummyflag -l -o -i $confpath $glstrings $euroweek" | tr -s ' '
        	        echo "$servicename stream tcp nowait root $tcpd $jaildir$glroot/bin/glftpd-full-static $dummyflag -l -o -i -e -I -n 0 -b -x -r $glroot/etc/glftpd.conf" | tr -s ' '
    } > /etc/inetd.conf.new
    ...
    ## SiCiaTeCh comments : change path for multi glftpd
...
service $servicename
{
    ...
    server_args     = ${tcpd:+$jaildir$glroot/bin/glftpd-full-static} -l -o -i -e -I -n 0 -b -x -r $glroot/etc/glftpd.conf
...
service "$servicename"
{
	...
	port "$servicename";
	user "root";
	exec "${tcpd:+$jaildir$glroot/bin/glftpd-full-static} $dummyflag -l -o -i -e -I -n 0 -b -x -r $glroot/etc/glftpd.conf";
	server "${tcpd:-$jaildir$glroot/bin/glftpd-full-static}";
...
ExecStart=-$jaildir$glroot/bin/glftpd-full-static -l -o -i -e -I -n 0 -b -x -r $glroot/etc/glftpd.conf
StandardInput=socket
SyslogFacility=ftp
EOF
## SiCiaTeCh comments : change max connextion to 300
